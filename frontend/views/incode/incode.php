...
<script>
    //通过ajax更新静态页用户登录状态
    $.getJSON("<?=\yii\helpers\Url::to(['member/head'])?>",function(json){
        //只处理登录状态
        if(json.isLogin){
            $("#user_info").html('欢迎'+json.username+',[<a href="register.html">注销</a>]');
            $("#user_info2").html('欢迎'+json.username+',[<a href="register.html">注销</a>]');
        }
    });
</script>